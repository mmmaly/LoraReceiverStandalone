...
| `-P` | **Maximum power**: `-g 47 -a -L 1.0`, applied only to whichever of those you did not set yourself | off |
| `-L <0-1>` | Digital amplitude. A LoRa chirp has 0 dB PAPR, so `1.0` is full DAC scale *without* clipping | 0.7 |
| `-O <hz>` | Transmit offset from the LO so its leakage falls outside the channel; `0` puts the LO on the channel. Every Hz costs signal to the DAC's sinc roll-off, so the default stays only as wide as it must be | 2x bw, min 100 kHz, capped at samp_rate/4 |
...
- **Constant envelope changes what "too much gain" means.** `build_upchirp` writes `cos`/`sin` of a phase, so `|z| = 1` for every non-silent sample: a LoRa chirp has 0 dB peak-to-average ratio. Nothing is gained by backing the DAC off — `-L 1.0` is full scale and still cannot clip — and nothing is lost by driving the analog chain into compression, which only costs harmonics and spectral regrowth, never signal quality. (This is why the SX1262's own PA runs saturated.) The old `-L 0.7` default threw away **3.1 dB** and used 89 of the DAC's 127 codes; the old `samp_rate/4` LO offset threw away another **0.9 dB** to sinc roll-off in order to move the LO spur 8x further out than a 62.5 kHz channel needs. `-P` takes back all of it plus the untouched VGA and amp headroom. What that overdrive *does* cost is out-of-band emissions, which are regulated separately from output power — an 868 MHz low-pass or SAW filter on the output is the price of running there.
- **The frame edges want a ramp.** Silence to full amplitude in one sample rings in the HackRF's reconstruction filter and splatters into neighbouring channels; at full scale that overshoot also clips. `lora_tx` now windows 20 µs of raised cosine onto each end of the burst — 1% of one of the eight preamble symbols, invisible to the demodulator. The `-o` file output keeps its hard edges, since the ringing belongs to the radio and not to the waveform.
...
**Transmitting is regulated.** The EU 868 MHz ISM band limits how long a device may occupy a channel, so `lora_tx` enforces a duty cycle by idling after each frame (default 1%, the conservative sub-band figure — 869.4–869.65 MHz permits 10% and 500 mW ERP) and never leaves the PA keyed between frames. A SF12/62.5 kHz frame is 2.3 s of airtime, which at 1% means ~4 minutes of silence after it; `-N` reports the airtime, the duty-cycle idle, the gain chain and the sinc loss before anything is radiated. Check what your license and local regulations allow before raising `-y`, `-g`, `-a` or `-P` — and note that the HackRF's own ceiling is only around +12 to +14 dBm at the SMA at 868 MHz, so the remaining dB to 100 mW EIRP come from the antenna or an external PA, not from the radio.
